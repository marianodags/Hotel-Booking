<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){	
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
							...
							while($f_date = $q_date->fetch_array()){
								echo "<ul>
										<li>	
											<label class = 'alert-danger'>".date("M d, Y", strtotime($f_date['checkin']."+6HOURS"))."</label>	
										</li>
									</ul>";
							}
						"</div>";
				...
				// 			$conn->query("INSERT INTO `transaction`(guest_id, room_id, status, checkin) VALUES('$guest_id', '$room_id', 'Pending', '$checkin')") or die('Error: ' . mysqli_error());
							
							$data ="INSERT INTO `transaction`(guest_id, room_id, status, checkin) VALUES('$guest_id', '$room_id', 'Pending', '$checkin')";
							$query = mysqli_query($conn, $data) or die('Error: ' . mysqli_error($conn));
